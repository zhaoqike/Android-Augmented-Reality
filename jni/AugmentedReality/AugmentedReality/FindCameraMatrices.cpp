					  ...
					  ) 
{
...
#endif
		cerr << "pts1 " << pts1.size() << " (orig pts " << imgpts1_tmp.size() << ")" << endl;
		cerr << "pts2 " << pts2.size() << " (orig pts " << imgpts2_tmp.size() << ")" << endl;
		double minVal,maxVal;
		minMaxIdx(pts1,&minVal,&maxVal);
		cout << "begin find fundamental mat: " << maxVal << "  " << 0.006*maxVal << endl;
		double reprojectionError = 10.0;
		F = findFundamentalMat(pts1, pts2, FM_RANSAC, reprojectionError, 0.99, status); //threshold from [Snavely07 4.1]
		//F = findFundamentalMat(pts1, pts2);//default parameters
		
	}
	
	vector<DMatch> new_matches;
	cout << "F keeping " << countNonZero(status) << " / " << status.size() << endl;	
	cerr << "F keeping " << countNonZero(status) << " / " << status.size() << endl;	
	...
	cerr << matches.size() << " matches before, " << new_matches.size() << " new matches after Fundamental Matrix\n";
...
bool TestTriangulation(const vector<CloudPoint>& pcloud, const Matx34d& P, vector<uchar>& status,float threshold) {
	...
	if(false) //not
	{
		Mat_<double> cldm(pcloud.size(),3);
		...
		int num_inliers = 0;
		Vec3d nrm = pca.eigenvectors.row(2); nrm = nrm / norm(nrm);
		Vec3d x0 = pca.mean;
	...
	Mat_<double>& t2) 
{
	...
	cout<<"singular value ratio: "<<singular_values_ratio<<endl;
						...
						) 
{
	...
	{
		cerr<<"find camera begin"<<endl;
		cout << "Find camera matrices..."<<endl;
		double t = getTickCount();
		//cerr<<imgpts1.size()<<endl;
		//Mat points1 = ((InputArray)imgpts1).getMat(), points2 = ((InputArray)imgpts2).getMat();
        //int npoints = points1.checkVector(2);
		//cerr<<npoints<<endl;
		cout<<"before fundamentalMat "<<matches.size()<<endl;
		Mat F = GetFundamentalMat(imgpts1,imgpts2,imgpts1_good,imgpts2_good,matches
#ifdef __SFM__DEBUG__
								  ,img_1,img_2
#endif
								  );
		cout<<"after fundamentalMat "<<matches.size()<<endl;

		if(matches.size() < 10) { // || ((double)imgpts1_good.size() / (double)imgpts1.size()) < 0.25
			cout << "not enough inliers after F matrix" << endl;
			return false;
		}
		cerr<<"before mul"<<endl;
		...
		if(fabsf(determinant(E)) > 1e-6){//1e-07) {
		...
		{			
			...
			if (!TestTriangulation(pcloud, P1, tmp_status, initTriangulationThreshold) || !TestTriangulation(pcloud1, P, tmp_status, initTriangulationThreshold) || reproj_error1 > initProjectionErrorThreshold || reproj_error2 > initProjectionErrorThreshold) {
