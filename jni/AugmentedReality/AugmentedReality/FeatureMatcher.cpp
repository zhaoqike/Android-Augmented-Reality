...
int FeatureMatcher::MaskMatch(KeyFrame& queryframe,KeyFrame& trainframe,vector<DMatch> &matches)
{
	...
	Timer timer;
	timer.start();
	double maskmatchstart=timer.getElapsedTimeInMilliSec();
	MaskMatch(queryframe.descriptors, trainframe.descriptors, matches );
	double maskmatchend = timer.getElapsedTimeInMilliSec();
	double maskmatchtime = maskmatchend - maskmatchstart;
	cout << "maskmatchtime cost: " << maskmatchtime << endl;
	cout << queryframe.descriptors.size() << "  " << trainframe.descriptors.size() << matches.size() << endl;
	cout<<"end match"<<endl;
	...
	cout<<"begin elliminate"<<endl;
	double ellistart = timer.getElapsedTimeInMilliSec();
	...
	double elliend = timer.getElapsedTimeInMilliSec();
	double ellitime = elliend - ellistart;
	cout << "elliminate time cost: " << ellitime << endl;
	cout<<"end elliminate    "<<matches.size()<<"  "<<good_matches_.size()<<endl;
	matches.swap(good_matches_);
	cout << "after swap matches" << matches.size() << "  " << good_matches_.size() << endl;
	cout << "keypoints_1.size() " << queryframe.keyPoints.size() << " imgpts1_good.size() " << imgpts1_good.size() << endl;
	cout << "keypoints_2.size() " << trainframe.keyPoints.size() << " imgpts2_good.size() " << imgpts2_good.size() << endl;
	...
	double fundstart = timer.getElapsedTimeInMilliSec();
	
	GetFundamentalMat(queryframe.keyPoints,trainframe.keyPoints,queryframe.keyPointsGood,trainframe.keyPointsGood,matches);
	double fundend = timer.getElapsedTimeInMilliSec();
	double fundtime = fundend - fundstart;
	cout << "get fundamental mat cost: " << fundtime << endl;
	cout << "after mask match: " << matches.size() << endl;
	return 0;
}

int FeatureMatcher::MaskMatch(Mat& queryDescriptors, Mat& trainDescriptors, vector<DMatch> &matches)
{
	//matching descriptor vectors using Brute Force matcher
	BFMatcher matcher(NORM_HAMMING, true); //allow cross-check. use Hamming distance for binary descriptor (ORB)
	vector< DMatch > good_matches_, very_good_matches_;
...
int FeatureMatcher::SymmetryMatch(KeyFrame& queryframe,KeyFrame& trainframe,vector<DMatch> &matches)
{
	...
	matches=symMatches;
	Mat fundemental= ransacTest(symMatches, queryframe.keyPoints, trainframe.keyPoints, matches);
	cout<<"symmatches: "<<symMatches.size()<<endl;
	cout<<"matches: "<<matches.size()<<endl;
...
int FeatureMatcher::Match(KeyFrame& queryframe,KeyFrame& trainframe,vector<DMatch> &matches)
{
	...
	double matchtime = matchend - matchstart;
	cout << "match keypoints cost: " << matchtime << endl;
