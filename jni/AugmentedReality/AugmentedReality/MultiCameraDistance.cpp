...
{		
	...
	for (unsigned int i=0; i<imgs_.size(); i++) {
		imgs_orig.push_back(Mat_<Vec3b>());
		if (!imgs_[i].empty()) {
		...
		imgpts.push_back(vector<KeyPoint>());
		imgpts_good.push_back(vector<KeyPoint>());
		cout << ".";
	}
	cout << endl;
	...
	FileStorage fs;
	if(fs.open(imgs_path_+ "\\out_camera_data.yml",FileStorage::READ)) {
		fs["camera_matrix"]>>cam_matrix;
		fs["distortion_coefficients"]>>distortion_coeff;
	} else {
		//no calibration matrix file - mockup calibration
		Size imgs_size = imgs_[0].size();
		double max_w_h = MAX(imgs_size.height,imgs_size.width);
		cam_matrix = (Mat_<double>(3,3) <<	max_w_h ,	0	,		imgs_size.width/2.0,
