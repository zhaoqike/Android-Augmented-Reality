...
void BundleAdjuster::adjustRecentBundle(CloudMap &cloudmap,int queryIndex,int trainIndex,vector<DMatch> &matches,Mat& cam_matrix)
{
	KeyFrame kfquery=cloudmap.keyFrames[queryIndex];
	KeyFrame kftrain=cloudmap.keyFrames[trainIndex];

	int N = 2, M = matches.size(), K = 2*M;
	...
	{
		int pointId = j;
		int idxInMap=kfquery.indexInMap[matches[j].queryIdx];
		Xs[j][0] = cloudmap.pcloud[idxInMap].pt.x;
		Xs[j][1] = cloudmap.pcloud[idxInMap].pt.y;
		Xs[j][2] = cloudmap.pcloud[idxInMap].pt.z;
		pointIdFwdMap[j] = pointId;
		pointIdBwdMap.insert(make_pair(pointId, j));
	}
	cout << "Read the 3D points." << endl;
	...
	Pmats.push_back(kfquery.Pmats);
	Pmats.push_back(kftrain.Pmats);
	vector<CameraMatrix> cams(N);
	for (int i = 0; i < N; ++i)
	{
		int camId = i;
	...
	for (unsigned int k = 0; k < matches.size(); ++k)
	{
		for (unsigned int i=0; i<N; i++) {
			...
			if (0==i) {
				...
				cvp = kfquery.keyPoints[matches[k].queryIdx].pt;
			}
			else
			{
				cvp = kftrain.keyPoints[matches[k].trainIdx].pt;
			}
			p[0] = cvp.x;
			p[1] = cvp.y;
			p[2] = 1.0;
...
//		StdMetricBundleOptimizer opt(inlierThreshold,cams,Xs,measurements,correspondingView,correspondingPoint);
	...
	if(good_adjustment) { //good adjustment?
		...
		for (unsigned int j = 0; j < Xs.size(); ++j)
		{
			...
			cloudmap.pcloud[idxInMap].pt.x = Xs[j][0];
			cloudmap.pcloud[idxInMap].pt.y = Xs[j][1];
			cloudmap.pcloud[idxInMap].pt.z = Xs[j][2];
		}
		...
		kfquery.Pmats=Pmats[0];
		kftrain.Pmats=Pmats[1];
...
int BundleAdjuster::CountRecent2DMeasurements(const vector<CloudPoint>& pointcloud) 
{
	return 0;
}